            ...
            <div style={{ textAlign: "center", padding:"5px" }}>
                <form>
                    {tops.map((k, i) => (
                        <Fragment key={k}>
                            <h3>{"#" + (i+1)} {star(i)} </h3>
                            <Top2023Search onSearchTermChange={(term) => handleSearchTermChange(i, term)}/>
                        </Fragment>
                    ))}
                    <br />
                    <div style={{ display: 'flex', flexDirection: 'column', padding: "10px", alignItems: 'center' }}>
                        <label htmlFor="othermovies" style={{ fontSize: "15px" }}> Quels autres films avez-vous particulièrement apprécié découvrir cette année ? (facultatif)</label>
                        <textarea
                            id="othermovies"
                            value={othermovies}
                            rows={5}
                            style={{fontSize: "15px", wordWrap: 'break-word', width: 'min(95%, 400px)', height: '100px', padding: '5px' }}
                            onChange={handleothermoviesChange}
                        />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', padding: "10px", alignItems: 'center' }}>
                        <label htmlFor="nombredefois" style={{ fontSize: "15px" }}> A combien estimez-vous le nombre de fois où vous êtes allés voir un film de patrimoine au cinéma cette année ? (facultatif)</label>
                        <textarea
                            id="nombredefois"
                            value={nombredefois}
                            rows={5}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', padding: "10px", alignItems: 'center' }}>
                        <label htmlFor="real" style={{ fontSize: "15px" }}>Y a-t-il des films/réalisateurs en particulier que vous aimeriez voir plus souvent programmés en salle ? (facultatif)</label>
                        <textarea
                            id="real"
                            value={real}
                            rows={5}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', padding: "10px", alignItems: 'center' }}>
                        <label htmlFor="autreinformation" style={{ fontSize: "15px" }}>Faites-nous part de vos retours sur notre projet ou sur notre site web ! (facultatif)</label>
                        <textarea
                            id="autreinformation"
